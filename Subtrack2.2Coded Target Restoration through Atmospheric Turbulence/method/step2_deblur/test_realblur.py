...
parser.add_argument('--result_dir', default='./results/deblurring/', type=str, help='Directory for results')
parser.add_argument('--dataset', default='RealBlur_J,RealBlur_R', type=str, help='Test Dataset') 
parser.add_argument('--weights', default='models/model_epoch_50.pth',
...
def expand2square_multi_in_single_out(timg,factor=16.0):
    _, _, h, w = timg.size()

    X = int(math.ceil(max(h,w)/float(factor))*factor)
      
    img = torch.zeros(1,30,X,X).type_as(timg) # 3, h,w
    mask = torch.zeros(1,1,X,X).type_as(timg)
...
if __name__ == "__main__":
    ...
    for dataset in args.dataset.split(','):
        rgb_dir_test = os.path.join(args.input_dir, dataset, 'test', 'ug2_2')
        ...
        test_loader  = DataLoader(dataset=test_dataset, batch_size=1, shuffle=False, drop_last=False, pin_memory=False)
        ...
        with torch.no_grad():
            for ii, data_test in enumerate(tqdm(test_loader), 0):
                ...
                # input_, mask = expand2square_multi_in_single_out(input_, factor=128) 
                #为了处理

                # restored,turbu_T,turbu = model_restoration(input_)
                restored= model_restoration(input_)
                restored = torch.masked_select(restored,mask.bool()).reshape(1,3,h,w)
                # restored = torch.masked_select(restored,mask.bool()).reshape(1,1,h,w)

                restored = torch.clamp(restored,0,1).cpu().numpy().squeeze().transpose((1,2,0))
                # restored = torch.clamp(restored,0,1).cpu().numpy().squeeze()
                
                print(filenames, restored.shape)
                restored_16 = img_as_ubyte(restored)
                utils.save_img(os.path.join(result_dir,filenames[0]+'.png'), restored_16)
                # np.save(os.path.join(result_dir,filenames[0]+'_out.npy'),restored_16)
                # utils.save_img_double(os.path.join(result_dir,filenames[0]+'.png'),restored)
