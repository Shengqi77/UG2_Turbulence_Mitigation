...
with torch.no_grad():
    ...
    # #test_realimage
    # for jj, data_test in enumerate(tqdm(test_loader), 0):
    #     input_ = data_test[0].cuda()
    #     filenames = data_test[1]
    #     with torch.cuda.amp.autocast():
    #         restored = model_restoration(input_)
    #                 #save
    #     for times in restored.shape[0]:
    #         restored_1 = restored[times,:,:,:]
    #         restored_1 = restored_1.permute(1,2,0)
    #         restored_1 = restored_1.cpu().numpy()
    #         restored_1[restored_1>1] = 1
    #         restored_1[restored_1<0] = 0
    #         plt.imsave("./test_real/results/" + str(times)+'.png',restored_1)
    for ii, data_val in enumerate((val_loader), 0):
        ...
        with torch.cuda.amp.autocast():
            restored = model_restoration(input_)
            #可视化restored
            restored_1 = restored[1,:,:,:]
            restored_1 = restored_1.permute(1,2,0)
            restored_1 = restored_1.cpu().numpy()
            restored_1[restored_1>1] = 1
            restored_1[restored_1<0] = 0
            plt.imsave("./evl_debug/restored_1.png",restored_1)
    ...
    for i, data in enumerate(tqdm(train_loader), 0): 
        ...
        if (i+1)%eval_now==0 and i>0:
            with torch.no_grad():
                ...
                print("[Ep %d it %d\t PSNR turbu: %.4f\t] ----  [best_Ep_turbu %d best_it_turbu %d Best_PSNR_turbu %.4f] " % (epoch, i, psnr_val_rgb,best_epoch,best_iter,best_psnr))
                with open(logname,'a') as f:
                    f.write("[Ep %d it %d\t PSNR turbu: %.4f\t] ----  [best_Ep_turbu%d best_it_turbu %d Best_PSNR_turbu %.4f] " \
