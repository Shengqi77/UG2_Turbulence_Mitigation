function outmap = mergstatcombmm(map,tvol,gvol);
...
for k=1:length(xpos);
    ...
    reg1t = max(1,featvolt(template1(:),:));
    reg2t = max(1,featvolt(template2(:),:));
    reg1g = max(1,featvolg(template1(:),:));
    reg2g = max(1,featvolg(template2(:),:));
